...
X_Y_labeled = cbind(subject_merged,Y_labeled,X_extract)
#write.table(X_Y_labeled, "cleanData_trainAndTestMerged.txt")

#5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject.
s = split(X_Y_labeled[,-3],list(X_Y_labeled$SubjectID,X_Y_labeled$Activity),drop=T)
tidyData = unlist(sapply(s,colMeans))
